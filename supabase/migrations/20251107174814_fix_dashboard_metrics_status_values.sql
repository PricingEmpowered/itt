/*
  # Fix Dashboard Metrics Status Values

  Updates the get_dashboard_metrics function to use correct status values
  that match the actual data in the quotes table.
  
  ## Changes
  - Use 'Approved' instead of 'Won'
  - Use 'Rejected' instead of 'Lost'
  - Update all status checks to match actual data
*/
...
BEGIN
  -- Revenue for last 12 months (Approved quotes)
  ...
  -- Active quotes (Draft + Sent + Pending + Under Review) last 12 months
  SELECT COUNT(*)
  INTO v_active_quotes_12m
  FROM quotes
  WHERE status IN ('Draft', 'Sent', 'Pending Approval', 'Under Review')
    AND created_at >= NOW() - interval '12 months';
  ...
  -- Win rate last 12 months (Approved / (Approved + Rejected))
  SELECT COALESCE(
  ...
  -- Average deal score (for approved quotes)
